...
var recordInfoMv=new Vue({
    ...
    methods:{
        loadDatas:function (pageNo) {
            var vm=this;
            $.ajax(path+"/admin/findUserRechargeInfos",{
                dataType:"json",
                data:{
                    pageNo:pageNo,
                ...
                success:function(data){
                    if(data){
                        ...
                        if(pageNo==1){//加载第一页数据的时才会加载相应的页数信息
                            ...
                            createRecordsPagnation(1,data.totalPages);//创建分页
                        }
...
function createRecordsPagnation(pageIndex,totalPages){
    $("#recordPageUl").html("");
    ...
    var options = {
        ...
        onPageChanged: function(event,oldPage,newPage){
            if (oldPage==newPage) {
                ...
                recordInfoMv.loadDatas(newPage);
            }
        }
    };
    $("#recordPageUl").bootstrapPaginator(options);
}