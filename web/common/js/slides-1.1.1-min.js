...
(function(a) {
	a.fn.olvSlides = function(b) {
		a.fn.olvSlides.option = {
			...
			fadeEasing: "",
			play: 5000,
			autoPlay:false,//自动播放，默认false,不播放
			pause: 2000,
			hoverPause: false,
		};
		...
		return this.each(function() {
				...
				if (!m) {
					...
					if (J === "slide") {
						...
						q.children(":eq(" + v + ")", B).css({
							zIndex: 8
						}).addClass("intro1").fadeIn(b.fadeSpeed, b.fadeEasing, function() {
			...
			if (b.thumb) {
				...
				C.children(":eq(0)").children().each(function(L) {
					...
					var like = a(this).find("img").attr("likecss") == 1?"dzed":"lkdz";
					var K = '<li><div class="imgWrap"><a href="' + J + '" title="' + I + '"><img src="' + N + '" alt="' + I + '" /></a></div><div class="dz"><span class="lksc" onclick="ignorelikeme(this)"></span><span class="'+ like +'" onclick="ilike(this)"></span></div><div class="textDesc"><div class="text">' + M + "</div></div></li>";
					a(this).data("data", L);
					q.append(K)
				});
			...
			a("." + b.prev, B).click(function(I) {
				I.preventDefault();
				if (C.children(":eq(0)").children().size()===1) {
					layer.msg(onetip);
				};
				if (b.autoPlay&&b.play) {
					l()
			...
			$(".control").find("ul li:first").addClass("intro1");
		})
	}
})
