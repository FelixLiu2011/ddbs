...
<script src="<%=path%>/common/js/platform/platformDictConsumeType.js"></script>
    ...
    <div class="w1100 jz">
        ...
        <div class="fr w860">
            ...
            <div class="content1 mt15">
                <div class="vip-cont">
                    <div class="zqfl tr mb20"><span class="on">充值记录</span> / <span>消费记录</span></div>
                    <div class="vip-table-con">


                        <div class="table" >
                            <ul class="firstUl firstUl1">
                                <li class="w300">交易单号</li>
                                <li class="w115">充值金额</li>
                                <li class="w135">状态</li>
                                <li class="w177 lastLi">交易时间</li>
                                <div class="clear"></div>
                            </ul>

                            <div class="th-bg" id="chargeTable">
                                <ul>
                                    <!-- <li class="w135">12356201</li>
                                    <li class="w115"><span class="c-ff8a00">$50</span></li>
                                    <li class="w135">充值成功 </li>
                                    <li class="w177 "> 邀请好友赠送 </li>
                                    <li class="w177 lastLi">2015-08-19 10:20</li>
                                    <div class="clear"></div> -->
                                </ul>

                            </div>
                            <div class="page"  id="chargeTablePage">
                        ...
                        <div class="table" style="display: none;">
                            <ul class="firstUl firstUl1">
                                <li class="w300">说明</li>
                                <li class="w177">消费金额</li>
                                <li class="w177 lastLi">交易时间</li>
                                <div class="clear"></div>
                            </ul>

                            <div class="th-bg" id="consumTable">
                                <ul>
                                    <!-- <li class="w177 "> 邀请好友赠送 </li>
                                    <li class="w115"><span class="c-ff8a00">$50</span></li>
                                    <li class="w177 lastLi">2015-08-19 10:20</li>
                            ...
                            <div class="page" id="consumTablePage">
...
<script type="text/javascript">
    var chargePageNo=1;  //页码
    var consumPageNo=1;

    var chargePageCount=1; //总页数
    var consumPageCount=1;
    ...
    /*充值记录*/
    function searchChargeData(pageNo) {
        $.post("http://www.gagahi.com:80/recharge/getRecordDatas",
                {
                    "pageNo":pageNo,
                    "pageSize":pageSize
                },
                function(data,textStatus){
                    var obj = eval(data);
                    parseChargePage(obj.attributes.page);
                    parseChargeList(obj.attributes.data);
                })

    }
    //填充列表数据
    function parseChargeList(list){
        if(list.length<=0){
            //$("#chargeTable").hide();
            $("#chargeTablePage").html("<p class='m15 tc'>暂无数据</p>");
            return;
        }else{
            //$("#chargeTablePage").html("<div class='tcdPageCode'> </div>");
        }

        var dataChargeStr="";
        for(var i=0;i<list.length;i++){
            var dataStr="<ul><li class='w300'>"+list[i].rechId+"</li><li class='w115'><span class='c-ff8a00'>$"+list[i].rechGoldreal+"</span></li><li class='w135'>"+stateConver(list[i].rechState)+"</li>"+
                    "<li class='w177 lastLi'>"+getLocalTime(list[i].rechOrdertime)+"</li><div class='clear'></div></ul>";
            dataChargeStr=dataChargeStr+dataStr;
        }
        $("#chargeTable").html(dataChargeStr);
    }

    //整理分页数据
    function parseChargePage(pageUtil){
        if(null!=pageUtil){
            chargePageCount=pageUtil.totalPage;
        }
        createChargePage();
    }
    ...
    function  createChargePage(){
        ...
        $(".tcdPageCode").createPage({
            pageCount : chargePageCount,  //总页数
            current : chargePageNo,  //当前页
            backFn : function(p) {
                console.log(p);
                chargePageNo=p;
                searchChargeData(p);
    ...
    //消费记录
    function searchConsumData(pageNo) {
        $.post("http://www.gagahi.com:80/consum/getConsumDatas",
                {
                    "pageNo":pageNo,
                    "pageSize":pageSize
                },
                function(data,textStatus){
                    var obj = eval(data);
                    parseConsumPage(obj.attributes.page);
                    parseConsumList(obj.attributes.data);
                })

    }
    //填充列表数据
    function parseConsumList(list){
        if(list.length<=0){
            //$("#chargeTable").hide();
            $("#consumTablePage").html("<p class='m15 tc'>暂无数据</p>");
            return;
        }else{
            //$("#consumTablePage").html("<div class='tcdPageCode1'> </div>");
        }
        var dataConsumStr="";
        for(var i=0;i<list.length;i++){

            //var desc = getPlatformDictConsume(list[i].consTypeparentid,list[i].consTypeparentid,list[i].consTypeparentdesc);
            var desc = converRemark(0==list[i].consTypeparentid?list[i].consTypeid:list[i].consTypeparentid);
            var dataStr1="<ul><li class='w300'>"+desc+"</li><li class='w177'><span class='c-ff8a00'>$"+list[i].consGoldreal+"</span></li>"+
                    "<li class='w177 lastLi'>"+getLocalTime(list[i].consTime)+"</li><div class='clear'></div></ul>";
            dataConsumStr=dataConsumStr+dataStr1;
        }
        $("#consumTable").html(dataConsumStr);
    }

    //整理分页数据
    function parseConsumPage(pageUtil){
        if(null!=pageUtil){
            consumPageCount=pageUtil.totalPage ;
        }
        createConsumPage();
    }

    function  createConsumPage(){
        /*分页*/
        $(".tcdPageCode1").createPage({
            pageCount : consumPageCount,//总页数
            current : consumPageNo, //当前页
            backFn : function(p) {
                console.log(p);
                consumPageNo=p;
                searchConsumData(p);
    ...
    //////////////////////////////////////////////////
    //充值状态
    function stateConver(state){
        var reslutstr="";
        if(state==0)  reslutstr="未支付";
        if(state==1)  reslutstr="充值成功";
        if(state==2)  reslutstr="充值失败";
        if(state==3)  reslutstr="已删除";
        if(state==4)  reslutstr="支付确认中";
        return reslutstr;
    }
    //
    //支出类型
    function converRemark(type){
        if(1==type)   return "红包";//"红包";
        if(2==type)   return "电商";//"电商";
        if(3==type)   return "会员升级";//"会员升级";
        if(4==type)   return "翻译";//"翻译";
        if(5==type)   return "私信翻译";//"私信翻译";
        if(6==type)   return "IM翻译";//"IM翻译";
        if(7==type)   return "动态翻译";//"动态翻译";
        if(8==type)   return "虚拟礼物";//"虚拟礼物";
        if(9==type)   return "表情";//"表情";
        if(10==type)  return "翻译包";//"翻译包";
        if(11==type)  return "商城消费";//"商城消费";
        if(12==type)  return "游戏";//"游戏";
        if(13==type)  return "展现天数";//"展现天数";
        if(14==type)  return "私信条数";//"私信条数";
        if(15==type)  return "主页照片展示张数";//"主页照片展示张数";
        if(17==type)  return "一键50位异性购买";//"一键50位异性购买";
        if(19==type)  return "聊天窗口购买";//"聊天窗口购买";
        if(21==type)  return "其他";//"其他";
        if(200==type) return "充值";//"充值";
        if(201==type) return "提现";//"提现";
    }
    ...
    function getLocalTime(timestamp) {
        ...
        timestamp=timestamp+""; //转换str
        var datestampStr="";
        if(timestamp.length>10){
            datestamp=timestamp;
        }else{
            datestamp=timestamp+"000"
        }
        var newDate = new Date();
        newDate.setTime(datestamp)
...
<script>
    ...
    $(window).load(function() {
        ...
        $(".th-bg>ul").each(function(index, ele) {
            if (index % 2 === 0) {
                $(this).css({
        ...
        $(".zqfl span").each(function(ix, e) {
            $(this).click(function() {
                $(this).addClass("on").siblings().removeClass("on");
                $(this).parent().next().children().eq(ix % 2).css("display", "block");
                $(this).parent().next().children().eq(ix % 2).siblings().css("display", "none");
            });
        })
    });
</script>
